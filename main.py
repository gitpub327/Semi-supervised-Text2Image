...
is_train = False
is_semi = True
unsup_rate = 0 # how many label ignored#
gpu_fraction = 0.7
critic_iteration = 5
...
## log
file = open(os.path.join(store_dir, 'settings.txt'), 'w')
file.write('')
file.write('=========== \n')
...
def main(unused_argv):
    ...
    image_unlabeled = tf.placeholder('float32', [batch_size, image_size, image_size, 1], name='image_unlabeled')
    image_unlabeled_wrong = tf.placeholder('float32', [batch_size, image_size, image_size, 1], name='image_unlabeled_wrong')
    ...
    _, ebd_real = image_encoder(image_labeled)
    _, ebd_i_wrong = image_encoder(image_wrong, reuse=True)
    
    _, fake_img = image_decoder(tf.concat([ebd_text, z], 1))

    _, d_real = discriminator(image_labeled, ebd_text)
    _, d_fake = discriminator(fake_img, ebd_text, reuse=True)
    # _, d_wrong = discriminator(image_wrong, ebd_text, reuse=True)
    _, d_wrong = discriminator(image_labeled, ebd_t_wrong, reuse=True)

    ## unsup part ##
    _, ebd_unlb = image_encoder(image_unlabeled, reuse=True)
    _, ebd_unlb_wrong = image_encoder(image_unlabeled_wrong, reuse=True)

    _, fake_unlb_img = image_decoder(tf.concat([ebd_unlb, z2], 1), reuse=True)

    _, d_unlb_real = discriminator(image_unlabeled, ebd_unlb, reuse=True)
    _, d_unlb_fake = discriminator(fake_unlb_img, ebd_unlb, reuse=True)
    _, d_unlb_wrong = discriminator(image_unlabeled, ebd_unlb_wrong, reuse=True)    
